...
EM2874Device* EM2874Device::AllocDevice(int idx)
{
	...
	if (idx == -1) {	// デバイス指定無し
		for (int i=0; i < 40; i++) {
			// デバイスインタフェースを列挙
			if (TRUE == SetupDiEnumDeviceInterfaces(deviceInfo, NULL, (GUID*)&GUID_WINUSB_DRIVER, i, &interfaceData)) {
				dwRet = ::GetLastError();
		...
		if (TRUE == SetupDiEnumDeviceInterfaces(deviceInfo, NULL, (GUID*)&GUID_WINUSB_DRIVER, idx, &interfaceData)) {
			dwRet = ::GetLastError();
...
bool EM2874Device::initDevice2 () const
{
	uint8_t val;
...
uint8_t EM2874Device::readReg (const uint8_t idx) const
{
	uint8_t val;
	readReg (idx, &val);
	return val;
...
bool EM2874Device::readReg (const uint8_t idx, uint8_t *val) const
{
...
bool EM2874Device::writeReg (const uint8_t idx, const uint8_t val) const
{
	WINUSB_SETUP_PACKET spkt;
	::ZeroMemory ( &spkt, sizeof(spkt) );
...
bool EM2874Device::readI2C (const uint8_t addr, const uint16_t size, uint8_t *data, const bool isStop) const
{
	WINUSB_SETUP_PACKET spkt;
	::ZeroMemory ( &spkt, sizeof(spkt) );
	spkt.RequestType = 0xc0;
...
bool EM2874Device::writeI2C (const uint8_t addr, const uint16_t size, uint8_t *data, const bool isStop) const
{
	WINUSB_SETUP_PACKET spkt;
	::ZeroMemory ( &spkt, sizeof(spkt) );
	spkt.RequestType = 0x40;
...
WINUSB_INTERFACE_HANDLE EM2874Device:: getUsbHandle() const {
	return(usbHandle);
}
...
int EM2874Device::getDeviceID() const
{
...
void EM2874Device::TransferPause() const
{
	writeReg( EM2874_REG_TS_ENABLE, 0 );
	if(hTsEvent == NULL) return;
...
void EM2874Device::TransferResume() const
{
...
int EM2874Device::DispatchTSRead(BOOL *bNextReadIn)
{
	int nRet;
	int cnt;
    if(bNextReadIn) *bNextReadIn = FALSE ;
	for(cnt = 0; ; cnt++) {
		if(bNextReadIn&&cnt>=RINGBUFF_SIZE-NUM_IOHANDLE) {
          // データ破壊の臨界点に到達(一旦制御を戻す)
			if (bNextReadIn) {
				*bNextReadIn = TRUE;
			}
          break ;
...
HANDLE EM2874Device::GetHandle() const
{ return hTsEvent; }
